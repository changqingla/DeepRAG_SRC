...
将解析好的向量按照RAGFlow原有逻辑存储到Elasticsearch中，但**无需租户系统和知识库层级**。

## 📋 前提条件

1. **Python环境**: Python 3.8+
2. **Elasticsearch**: 本地运行的ES实例
3. **向量数据**: 已通过embedding模块生成的向量文件
...
# 如果ES不在localhost
python simple_store.py ../embedding/embedded_chunks.json \
  --es-host http://192.168.1.100:9200 \
  --index my_vectors
```
...
# 查看索引mapping
curl http://localhost:9200/ragflow_vectors/_mapping

# 搜索数据
curl -X GET "http://localhost:9200/ragflow_vectors/_search?pretty" \
